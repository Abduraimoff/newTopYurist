...
class AuthRepository {
  ...
  Future<PhoneVerifiedResponse> verifyPhone(BuildContext context) async {
    try {
      final response = await request.doPostRequest(
          slug: "/api/auth",
          queryParameters: {
          ...
          options: Options(headers: {
            "Client-Authorization":
                "Basic ZGV2OjhmZGE5MTE1LTc3ZDktOWNiMS1iYWRjLWY4NmQyNmVlYzIwYw==",
          }));
  ...
  Future<ProfileResponse> sendOtpCode(BuildContext? context) async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        await _tokenProvider.saveAccessToken(data.credentials?.accessToken);
        await _tokenProvider.saveRefreshToken(data.credentials?.refreshToken);
        // await storage.write(
        //     key: Config.accessToken,
        //     value: "Bearer ${data.credentials?.accessToken}");
        // await storage.write(
        //     key: Config.refreshToken, value: data.credentials?.refreshToken);
      ...
      if (response.statusCode == 400) {
        return ProfileResponse(
            error: response.data["error"], statusCode: response.statusCode);
      }
      return response.data;
